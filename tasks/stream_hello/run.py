...
from faasmctl.util.planner import reset_batch_size
...
def worker_thread(end_time):
    ...
    while time.time() < end_time:
        try:
            result, func_metrics = send_message_and_get_result(size=50)
...
def test(ctx, batchsize=0):
    ...
    end_time = time.time() + 30  # Running for 5 minutes
    total_count = 0
    total_time = 0
    worker_num = 20
    if batchsize > 0:
