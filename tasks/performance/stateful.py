...
def generate_input_data(size):
    return [{"input": "none"} for _ in range(size)]


def send_message_and_get_result(input_data, chainedId_list):
    msg = {
        "user": "stream",
        "function": "stateful_exp",
    }
...
def worker_thread(end_time, atomic_count, inputbatch):
    count = 0
    total_time = 0
    results = []
    while time.time() < end_time:
        try:
            ...
            input_data = generate_input_data(inputbatch)
...
@task(default=True)
def run(ctx, batchsize=1, concurrency = 0, inputbatch = 1):
    """
    ...
    register_function_state("stream_stateful_exp", "", "")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=WORKER_NUM) as executor:
        futures = [executor.submit(worker_thread, end_time, atomic_count, inputbatch) for _ in range(WORKER_NUM)]
