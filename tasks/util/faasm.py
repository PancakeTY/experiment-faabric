...
from faasmctl.util.invoke import (
    invoke_wasm_messages as faasmctl_invoke_wasm_messages,
)
...
def get_faasm_exec_chained_milli_time_from_json(results_json, check=False):
    ...
    for result in results_json:
        ...
        worker_execute_elapse = (
            worker_execute_end_time - worker_execute_start_time
        )
        total_elapse = worker_execute_end_time - planner_queue_time

        function_metrics[function_name]["planner_queue_elapse"].append(
            planner_queue_elapse
        )
        function_metrics[function_name]["planner_consumed_elapse"].append(
            planner_consumed_elapse
        )
        function_metrics[function_name]["worker_queue_elapse"].append(
            worker_queue_elapse
        )
        function_metrics[function_name]["executor_prepare_time"].append(
            executor_prepare_time
        )
        function_metrics[function_name]["worker_execute_elapse"].append(
            worker_execute_elapse
        )
...
def get_faasm_metrics_from_json(
    json_result, deadline, function_include=None, native=False
):
    # Group the results by chain ID and find the actual_time for each chained functions
    grouped_results = defaultdict(list)
    function_metrics = defaultdict(lambda: defaultdict(list))
    ...
    for msg_result in json_result:
        ...
        if (
            msg_finish_ts is None
            or int(msg_result["finish_ts"]) > msg_finish_ts
        ):
        ...
        if native:
            ...
            planner_dispatch_time = int(
                msg_result.get("plannerDispatchTime", 0)
            )
            worker_queue_time = int(msg_result.get("workerQueueTime", 0))
            worker_pop_time = int(msg_result.get("workerPopTime", 0))
            executor_prepare_time = int(
                msg_result.get("ExecutorPrepareTime", 0)
            )
            worker_execute_start_time = int(
                msg_result.get("workerExecuteStart", 0)
            )
            worker_execute_end_time = int(
                msg_result.get("workerExecuteEnd", 0)
            )
        else:
        ...
        if avg_tuple_duration is not None:
            function_metrics[function_name]["avg_tuple_duration"].append(
                avg_tuple_duration
            )

    if msg_start_ts is not None and msg_finish_ts is not None:
        function_metrics["application"]["msg_latency"].append(
            (msg_finish_ts - msg_start_ts) * 1000
        )
...
def post_async_msg_and_get_result_json(
    msg,
    num_message=1,
    host_list=None,
    req_dict=None,
...
def post_async_batch_msg_and_get_result_json(
    msg, batch_size=100, host_list=None
):
    result = faasmctl_invoke_wasm(
        msg, num_messages=batch_size, dict_out=True, host_list=host_list
    )
...
def post_async_batch_msg(
    ...
    if batch_size != len(chained_id_list):
        ...
        assert False
    chained_id_list = faasmctl_invoke_wasm_messages(
        app_id,
        msg_dict=msg,
        num_messages=batch_size,
        input_list=input_list,
        chained_id_list=chained_id_list,
        num_retries=10000,
        ...
        for func_name, metrics in function_metrics.items():
            ...
            for metric_name, times in metrics.items():
                if metric_name == "count":
                    ...
                    log_file.write(
                        f"  Total count {metric_name}: {metric_value}\n"
                    )
                else:
                    average_metric_time = (
...
def generate_input_data(records, start, size, input_map):
    ...
    return [
        {key: record[input_map[key]] for key in input_map}
...
def get_result_thread(
    ...
    while True:
        ...
        start_ts = int(
            min(
                [
                    result_json["start_ts"]
                    for result_json in filtered_json_results
                ]
            )
        )
        with start_ts_lock:
            if (
                shared_batches_min_start_ts[0] is None
                or start_ts < shared_batches_min_start_ts[0]
            ):
...
def statistics_result(
    batches_result, DURATION, function_include=None, native=False
):
    function_metrics = defaultdict(lambda: defaultdict(list))
    actual_times_array = []
...
def generate_all_input_batch(records, inputbatch, input_map, input_msg):
