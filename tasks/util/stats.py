...
def extract_data(file_path, function_include=None):
    ...
    total_messages_pattern = re.compile(
        r"Total messages processed: (?P<total_messages>\d+),"
    )
    median_time_pattern = re.compile(
        r"Median actual time: (?P<median_time>[\d\.]+) ms,"
    )
    percentile_95_time_pattern = re.compile(
        r"95th percentile actual time: (?P<percentile_95_time>[\d\.]+) ms,"
    )
    percentile_time_pattern = re.compile(
        r"99th percentile actual time: (?P<percentile_time>[\d\.]+) ms,"
    )
    actual_duration_pattern = re.compile(
        r"Actual Duration: (?P<actual_duration>[\d\.]+) ms"
    )
    ...
    total_count_pattern = re.compile(
        r"  Total count count: (?P<total_count>[\d\.]+)"
    )
    average_duration_pattern = re.compile(
        r"  Average duration: (?P<average_duration>[\d\.]+) μs"
    )
    average_tuple_duration_pattern = re.compile(
        r"  Average avg_tuple_duration: (?P<avg_tuple_duration>[\d\.]+) μs"
    )
    ...
    runs = re.split(
        r"\n(?=\d{2}--\w{3}--\d{4} \d{2}:\d{2}:\d{2} Running with rate=)",
        log_text,
    )
    ...
    for run in runs:
        ...
        if total_messages:
            run_data["Total Messages Processed"] = int(
        ...
        if median_time:
            run_data["Median Actual Time (ms)"] = float(
                median_time.group("median_time")
            )

        percentile_95_time = percentile_95_time_pattern.search(run)
        if percentile_95_time:
            run_data["95th Percentile Actual Time (ms)"] = float(
                percentile_95_time.group("percentile_95_time")
            )

        # Extract 99th percentile actual time
        percentile_time = percentile_time_pattern.search(run)
        if percentile_time:
            run_data["99th Percentile Actual Time (ms)"] = float(
                percentile_time.group("percentile_time")
            )

        # Extract actual duration
        actual_duration = actual_duration_pattern.search(run)
        if actual_duration:
            run_data["Actual Duration (ms)"] = int(
                actual_duration.group("actual_duration")
            )
        ...
        for function_match in function_pattern.finditer(run):
            ...
            function_metrics = function_match.group(
                0
            )  # The whole match including the data

            # Extract total count
            total_count_match = total_count_pattern.search(function_metrics)
            total_count = (
                int(total_count_match.group("total_count"))
                if total_count_match
                else None
            )

            # Extract average duration
            average_duration_match = average_duration_pattern.search(
                function_metrics
            )
            average_duration = (
                float(average_duration_match.group("average_duration"))
                if average_duration_match
                else None
            )

            # Extract average tuple duration
            avg_tuple_duration_match = average_tuple_duration_pattern.search(
                function_metrics
            )
            avg_tuple_duration = (
                float(avg_tuple_duration_match.group("avg_tuple_duration"))
                if avg_tuple_duration_match
                else None
            )
        ...
        if function_include is not None:
            ...
            for function_name, metrics in run_data["Functions"].items():
                if function_include + "_" in function_name:
                    run_data["Total Messages Processed"] = (
                        run_data["Total Messages Processed"]
                        + metrics["Total Count"]
                    )
...
def extract_avg_tuple_duration(func_data, function_name):
    if isinstance(func_data, dict):
        return func_data.get(function_name, {}).get(
            "Average Tuple Duration (µs)"
        )
    else:
        try:
            func_dict = ast.literal_eval(func_data)
            return func_dict.get(function_name, {}).get(
                "Average Tuple Duration (µs)"
            )
        except Exception:
            return None
...
def extract_avg_tuple_duration_prefix(func_data, function_name_prefix):
    ...
    if isinstance(func_data, dict):
        ...
        for key, value in func_data.items():
            if (
                key.startswith(function_name_prefix)
                and "Average Tuple Duration (µs)" in value
            ):
...
def print_data(data):
    ...
    print("-" * 120)
    print(
        "{:<12} {:<11} {:<11} {:<6} {:<25} {:<22} {:<32} {:<20} {:<20}".format(
            "Input Rate",
            "Batch Size",
            "Concurrency",
            "Scale",
            "Total Messages Processed",
            "Median Actual Time (ms)",
    ...
    for entry in data:
        print(
            "{:<12} {:<11} {:<11} {:<6} {:<25} {:<22} {:<32} {:<20} {:<20} {:.2f}".format(
...
def parse_log(path):
    ...
    run_delimiter_pattern = re.compile(
        r"\d{2}--\w{3}--\d{4} \d{2}:\d{2}:\d{2} Running with"
    )
    ...
    for chunk in log_chunks:
        ...
        if params_match and json_start_index != -1:
            ...
            try:
                ...
                print(
                    f"Warning: Could not parse JSON in a chunk:\n{chunk[:100]}..."
                )
...
def average_metrics(df):
    statistics_df = df.groupby(["rate", "schedulemode"])[
        ["throughput", "p99Latency", "p95Latency", "medianLatency"]
    ].mean()
